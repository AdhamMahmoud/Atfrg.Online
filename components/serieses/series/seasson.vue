        ...
        <div :class="{ col_show : active == 'trailer' , col_hide : active != 'trailer' }" id="trailer">
          <!-- Trailer Player -->
            <vue-plyr class="player-mov player-trailer" ref="e3lan" :options="playerOptions">
             <div :id="seasons[0].trailerPath" data-plyr-provider="youtube" :data-plyr-embed-id="seasons[0].trailerPath"></div>
            </vue-plyr>
        </div>
        <!-- Movie -->
        <div :class="{ col_show : active == 'movie' , col_hide : active != 'movie' }" id="movie">
          ...
          <!-- Movie Player -->
            <vue-plyr
                ...
                <video>
                  <!-- Video Source -->
                  <source v-for="video in episodes[0].links" :key="video.id" :src="video.path" type="video/webm" :size="video.quality.replace('Q','')" >
                  <!-- Video Subtitles -->
                  <track v-for="(subtitle, index) in episodes[0].subtitles" :key="subtitle.id" kind="captions" :label="subtitle.name" :srclang="subtitle.lang.name"
                    :src="subtitle.path" :default="{ 'default': index == episodes[0].subtitles.length - 2}">
                </video>
            </vue-plyr>
          </div>
          <!-- Download -->
          <div :class="{ col_show : active == 'download' , col_hide : active != 'download' }" id="download">
            <ul class="download-item">
               <li v-for="video in episodes[0].links" :key="video.id"><nuxt-link :to="video.path">{{video.quality.replace('Q','')}}</nuxt-link> </li>
            </ul>
          </div>
      </div>
      </div>
